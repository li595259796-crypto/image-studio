...
export const copy: DeepReadonly<Record<Locale, LocaleCopy>> = deepFreeze({
  zh: {
    ...
    settings: {
      ...
      localeFailed: '语言切换失败',
      apiKeys: {
        loadingLabel: '正在加载 API 密钥…',
        retryLabel: '重试',
        loadFailed: 'API 密钥加载失败',
        saveSuccess: 'API 密钥已保存',
        saveFailed: 'API 密钥保存失败',
        deleteSuccess: 'API 密钥已删除',
        deleteFailed: 'API 密钥删除失败',
        testUnavailable: 'API 密钥验证功能将在后续版本中上线。',
        summaryTitle: '自带 API 密钥',
        summaryDescription: '接入你自己的模型凭据，让工作台直接调用你配置的供应商。',
        betaLabel: '测试版',
        fairUseLabel: '合理使用',
        fairUseDescription: '使用自有密钥的调用不计入平台额度，但仍需遵守合理使用限制与服务端安全检查。',
        currentKeyLabel: '当前密钥',
        configuredLabel: '已配置',
        unconfiguredLabel: '未配置',
        emptyKeyLabel: '尚未保存密钥',
        inputLabel: '新 API 密钥',
        inputPlaceholder: '粘贴 API 密钥',
        saveLabel: '保存',
        savingLabel: '正在保存…',
        deleteLabel: '删除',
        deletingLabel: '正在删除…',
        ...
        providers: {
          google: {
            ...
            description: '使用自己的密钥调用 Gemini Flash 图像生成。',
          },
          bytedance: {
            title: 'ByteDance Ark',
            description: '使用自己的 Ark API Token 调用 Seedream 生成。',
          },
          alibaba: {
            title: 'Alibaba DashScope',
            description: '使用自己的 DashScope 密钥调用通义万相生成。',
          },
        },
      },
    },
